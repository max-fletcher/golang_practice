...
func chargeForLineItem[T lineItem](newItem T, oldItems []T, balance float64) ([]T, float64, error) {
	...
	if newBalance < 0.0 {
		return nil, 0.0, errors.New("insufficient funds") // Not sure why this has to be nil not oldItems. If its not nil, 2nd test doesn't pass.
	}
...
func main() {
	...
	fmt.Println("---------------------------------")
	fmt.Printf("%d passed, %d failed", passCount, failCount)
